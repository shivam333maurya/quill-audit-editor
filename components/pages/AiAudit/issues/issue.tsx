...
import { IIssuesType } from "@/utils/interfaces/interfaces";
import { IIssuePropsType } from "@/utils/interfaces/props-interfaces";
import { HrDivider } from "@/components";

const Issue = ({ selectedIssue, onClick }: IIssuePropsType) => {
  return (
    <div className="h-full overflow-auto">
      ...
      {selectedIssue?.issues?.map((item: IIssuesType, index: number) => {
        return (
          <div
            key={`${item?.issue} ${index + 1}`}
            onClick={() =>
              onClick &&
